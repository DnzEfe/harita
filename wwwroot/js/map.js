require([
    ...
    "esri/core/reactiveUtils" // GÜNCELLEME: Yeni dinleme modülü eklendi
], function (Map, MapView, SceneView, Graphic, GraphicsLayer, Fullscreen, BasemapGallery, Expand, Basemap, Search, DistanceMeasurement2D, DirectLineMeasurement3D, Daylight, Home, GeoJSONLayer, LayerList, TileLayer, Weather, reactiveUtils) { // GÜNCELLEME: Parametre eklendi
    ...
    <div id="tesisFormKutusu" style="display:none; position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); background:white; padding:20px; z-index:99; box-shadow:0 4px 15px rgba(0,0,0,0.3); border-radius:8px; width: 300px; font-family: sans-serif;">
        ...
        <label>Tesis Türü:</label><br>
        <select id="tTuru" style="width:100%; margin-bottom:10px; padding:5px;">
    ...
    const tesisPopupSablonu = {
        title: "{Name}",
        content: "<b>Tesis Türü:</b> {Type} <br/> <b>Kurulu Güç:</b> {Power} MW <br/> <b>İl:</b> {City} <br/> <b>Enlem:</b> {Lat} <br/> <b>Boylam:</b> {Lon}",
        actions: [{
            title: "Tesisi Sil",
            id: "delete-tesis",
    ...
    function handlePopupAction(event) {
        if (event.action.id === "delete-tesis") {
            ...
            if (confirm("Bu tesisi kalıcı olarak silmek istediğinize emin misiniz?")) {
                fetch('/api/tesis/' + dbId, { method: 'DELETE' })
                    .then(res => {
                        if (res.ok) {
                            tesislerKatmani.remove(seciliGrafik);
    ...
    // GÜNCELLEME: reactiveUtils ile yeni sisteme uygun dinleme yöntemi
    ...
    document.getElementById("btnKaydet").addEventListener("click", () => {
        const yeniTesis = {
            tesisAdi: document.getElementById("tAdi").value,
            tesisTuru: document.getElementById("tTuru").value,
            kuruluGuc: parseFloat(document.getElementById("tGuc").value) || 0,
            il: document.getElementById("tIl").value,
            enlem: parseFloat(document.getElementById("tEnlem").value),
            boylam: parseFloat(document.getElementById("tBoylam").value)
        };
        ...
        fetch('/api/tesis', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(yeniTesis) })
            ...
            .then(responseObj => {
                ...
                let sembolRengi;
                switch (yeniTesis.tesisTuru) {
                    case "HES": sembolRengi = [0, 112, 255]; break; case "GES": sembolRengi = [255, 204, 0]; break;
                    case "RES": sembolRengi = [50, 205, 50]; break; case "Termik": sembolRengi = [105, 105, 105]; break;
                    default: sembolRengi = [255, 0, 0];
                }
                ...
                const pointGraphic = new Graphic({
                    ...
                    attributes: {
                        Id: kaydedilenVeri.id,
                        Name: kaydedilenVeri.tesisAdi, Type: kaydedilenVeri.tesisTuru, Power: kaydedilenVeri.kuruluGuc,
                        City: kaydedilenVeri.il, Lat: kaydedilenVeri.enlem, Lon: kaydedilenVeri.boylam
                    },
    ...
    fetch('/api/tesis?' + new Date().getTime()).then(res => res.json()).then(kayitliTesisler => {
        kayitliTesisler.forEach(tesis => {
            const tId = tesis.id || tesis.Id;
            const tTuru = tesis.tesisTuru || tesis.TesisTuru;
            const tAdi = tesis.tesisAdi || tesis.TesisAdi;
            const tGuc = tesis.kuruluGuc || tesis.KuruluGuc;
            const tIl = tesis.il || tesis.Il;
            const tLat = tesis.enlem || tesis.Enlem;
            const tLon = tesis.boylam || tesis.Boylam;
            ...
            const pointGraphic = new Graphic({
                geometry: { type: "point", longitude: tLon, latitude: tLat },
                symbol: { type: "simple-marker", color: sembolRengi, outline: { color: [255, 255, 255], width: 1.5 }, size: "14px" },
                attributes: { Id: tId, Name: tAdi, Type: tTuru, Power: tGuc, City: tIl, Lat: tLat, Lon: tLon },
                popupTemplate: tesisPopupSablonu
            });
            tesislerKatmani.add(pointGraphic);
        });
    ...
    // --- HARİTAYA TIKLAMA (İLİ OTOMATİK BULMA) ---
    function handleMapClick(event) {
        ...
        ilSinirlariLayer.queryFeatures(query).then(function (response) {
            if (response.features.length > 0) {
                document.getElementById("tIl").value = response.features[0].attributes.name;
            } else {
                document.getElementById("tIl").value = "Deniz / Sınır Dışı";
            }

            tesisFormKutusu.style.display = "block";
            tesisEklemeModu = false;
            tesisEkleBtn.style.backgroundColor = "";
            document.body.style.cursor = "default";
        }).catch(() => {
            document.getElementById("tIl").value = "Bilinmiyor";
            tesisFormKutusu.style.display = "block";
        });
    }
